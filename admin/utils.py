...
def get_total_storage_usage():
    """
    현재 전체 업로드 파일 용량 계산
    Returns: 총 사용 용량 (bytes)
    """
    total = db.session.query(db.func.sum(File.size)).scalar()
    return total or 0


def check_storage_available(new_file_size):
    """
    새 파일 업로드 시 전체 용량 제한 확인
    Args:
        new_file_size: 업로드할 파일 크기 (bytes)
    ...
    if current_usage + new_file_size > max_storage:
        ...
        return False, f"전체 저장 공간이 부족합니다. (현재: {used_gb:.2f}GB / 최대: {max_gb:.0f}GB)"
